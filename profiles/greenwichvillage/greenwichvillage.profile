...
function greenwichvillage_form_install_configure_form_alter(&$form, $form_state) {
  $form['site_information']['site_name']['#default_value'] = t('Greenwichvillage');
  $form['#submit'][] = 'greenwichvillage_install_configure_form_submit';
}

/**
 * Forms API submit for the site configuration form.
 */
function greenwichvillage_install_configure_form_submit($form, &$form_state) {
  global $user;
  $sql_file = dirname(__FILE__).'/sample_content.sql';
  $count = greenwichvillage_import_sql($sql_file);
  drupal_set_message("Your greenwichvillage Drupal installation was successful.");
  ...
  $enable = array(
	  'theme_default' => 'greenwichvillage',
	  'admin_theme' => 'seven',
	  //'zen'
  );
  theme_enable($enable);
...
function greenwichvillage_import_sql($filename){
  global $databases;
  if (@mysql_connect($databases['default']['default']['host'], $databases['default']['default']['username'], $databases['default']['default']['password'])){
    ...
    if ($handle) {
      while (!feof($handle)) {
        ...
        if(preg_match('|;$|', $line)){
          $count++;
          mysql_query(greenwichvillage_prefixTables($buffer));
          $buffer='';
        }
      }
      fclose($handle);
...
function greenwichvillage_prefixTables($sql) {
  global $databases;
